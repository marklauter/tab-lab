...
public static class TabPlayer
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        // Parse the tablature into a sequence of playable "moments" (notes/chords)
        var moments = ParseTab(tab);
        Console.WriteLine($"Parsed {moments.Count} moments from the tab.");

        // Play the parsed moments
        await PlayMoments(moments);

        Console.WriteLine("Playback finished. Press any key to exit.");
        _ = Console.ReadKey();
    }

    // Represents a single note with its frequency and duration
    private record Note(float Frequency, float Duration);

    // Parses the multi-line tab string into a list of "moments".
    // Each moment contains the notes that should be played at the same time.
    private static List<List<Note>> ParseTab(string tab)
    {
    ...
    private static async Task PlayMoments(List<List<Note>> moments)
    {
